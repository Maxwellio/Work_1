...
import { userOrgApi } from '../../../../../utils/api';
...
export const useFetchOrg = <T,> () => {
    const [org, setOrg] = useState<T[]>([]);
    const [loagingOrg, setLoading] = useState<boolean>(false);

    useEffect(() => {
        const fetchData = async () => {     
           
            if (loagingOrg) return;
            setLoading(true);
            try {
                const reqApi = userOrgApi;
                console.log(reqApi);               
